...
def_viz <- function(folder, mes_inicial, mes_final, ano_final, ano_inicial, annual) {
  ...
  for (yr in ano_inicial:ano_final) {
    ...
    dir.create(file.path(folder, "outputs", yr), recursive = TRUE,
               showWarnings = FALSE)
    ...
    for (i in months) {
      ...
      # Define priorizacao vaiza (NA) como LOW
      ...
      patches_touching1 <- d |>
        ...
        dplyr::distinct() |>
        dplyr::mutate(area_m2 = as.numeric(sf::st_area(.data$geometry))) |>
        sf::st_drop_geometry()
      ...
      grDevices::png(file.path(save_path, paste0("v", i), paste0("erro_comissao_MAPA_", i, ".png")),
                     width = 3000, height = 2400, res = 300)
      ...
      prio_summary <- prio2 |>
        ...
        sf::st_drop_geometry() |>
        dplyr::mutate(month = if (annual == T) 1 else i)
      ...
      area_by_region <- patches_touching1 |>
        ...
        dplyr::mutate(ref_yer = if (annual == T) 1 else i, comission_rate = commission_rate)
...
# def_viz <- function(folder, mes_inicial, mes_final, ano_final, ano_inicial) {
#
#   sf::sf_use_s2(FALSE)
#
...
#
#     dir.create(file.path(folder, "outputs", year), recursive = TRUE,
#                showWarnings = FALSE)
#
...
#
#       #Previsao
#       #model <- readRDS(file.path(save_path,paste0("v",i),"final_model.rds"))
#       #dfinal <- read.csv(file.path("C:/Users/luktr/Desktop/lucas/testa_pacote/tabelas",
#       #                             paste0("df_operacionalizado_previsao_",
#       #                                    ano_inicial, "_", ano_final, ".csv")))
...
#       d <- deter |>
#         dplyr::filter(
#           lubridate::year(VIEW_DATE) == ano_final &
#             lubridate::month(VIEW_DATE) == i &
#             CLASSNAME %in% c("DESMATAMENTO_CR", "DESMATAMENTO_VEG", "MINERACAO")
#         )
#       #}
