...
#'
#' @export
update_deforestation <- function() {

  # 1. Load baseline from GitHub release

  base <- get_deforestation() |> dplyr::rename(geometry = .data$geom)
  # 2. Find last date in local dataset
  base$VIEW_DATE <- as.Date(base$VIEW_DATE)
  last_date <- max(base$VIEW_DATE, na.rm = TRUE)
  ...
  if (length(pieces) > 0) {
    ...
    cat("foi encontrado novos alertas do deter")
